...
public class StudentsManager: Manageable {
    ...
    public func insertStudent(_ student: Student?) throws {
        guard let student else {
            throw ManageError.studentNotAddedError
        }
        if (students.count < maxStudents){
            students.append(student)
        } else {
            throw ManageError.maxStudentsReachedError(max: maxStudents)
        }
        
    }
    
    public func asignSubjectToStudent(subject: Subject, score: Double, student: Student?) throws  {
        // Emplear más codigo despues de nuestra validacion
        if let student {
            for s in students {
                if student.email.elementsEqual(s.email) { // Equal -> ==
                    s.assignSubject(subject: subject, score: score)
                }
            }
        } else {
            throw ManageError.subjectNotAssignedError
        }
    }
    
    public func generateStudentsReport() throws {
        if students.isEmpty {
            throw ManageError.reportNotFoundError
        } else {
